...
"""
This segmentation approach doesn't show pauses at the segment boundary. 
It represents the natural flow of the signer
"""
...
class BiomechanicalSignProcessor:
    ...
    def predict_segment_async(self, features, prediction_queue):

        """
        Makes predictions asynchronously based on the provided features and places the result in the prediction queue.
                
        Args:
            features (list or np.ndarray): The features of the current segment to be predicted.
            prediction_queue (Queue): The queue to which the prediction result will be sent.
...
def analyze_sign_sequence(video_path, processor, kinematic_rest_threshold=0.85, rest_duration_threshold=8, bilateral_sync_threshold=0.2):
    ...
    while True:
            ...
            if signing_state == "ACTIVE":
                if segment_start_frame is not None and current_kinematic_features:
                    if prediction_thread is None or not prediction_thread.is_alive():
                        features_sequence = np.array(current_kinematic_features)
                        prediction_thread = threading.Thread(
                            target=processor.predict_segment_async,
                            args=(features_sequence, prediction_queue)
                        )
        ...
        try:
            while True:  
                pred_result = prediction_queue.get_nowait()
                if pred_result:
    ...
    while not prediction_queue.empty():
        pred_result = prediction_queue.get()
        if pred_result:
            detected_signs.append(pred_result['top_prediction'])
            segment_analysis.append({
                'segment': len(detected_segments) + 1,
