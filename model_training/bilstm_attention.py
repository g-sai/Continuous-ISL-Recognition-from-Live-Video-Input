...
from tensorflow.keras.layers import (
    Add, BatchNormalization, Bidirectional, Concatenate,
    Conv1D, Dense, Dropout, GlobalAveragePooling1D, Input,
    LSTM, Lambda, Layer, Multiply
)
...
def build_model(feature_dim, num_classes):
    ...
    """
    Builds the deep learning model for sequence classification.
    ...
    upper_body = Lambda(lambda x: x[:, :, :24])(inputs)
    left_hand = Lambda(lambda x: x[:, :, 24:87])(inputs)
    right_hand = Lambda(lambda x: x[:, :, 87:])(inputs)

    def create_feature_processor(input_tensor, units, name):
        x = Conv1D(filters=units, kernel_size=7, padding='same', activation='relu', name=f"{name}_conv1d")(input_tensor)
        x = BatchNormalization(name=f"{name}_batch_norm1")(x)

        attention_output = SelfAttention(units)(x)
        x = Add(name=f"{name}_add")([x, attention_output])
        x = BatchNormalization(name=f"{name}_batch_norm2")(x)
        
        return x

    upper_body_features = create_feature_processor(upper_body, 16, "upper_body")
    left_hand_features = create_feature_processor(left_hand, 32, "left_hand")
    right_hand_features = create_feature_processor(right_hand, 32, "right_hand")

    x = Concatenate(name="concatenate")([upper_body_features, left_hand_features, right_hand_features])

    x = Bidirectional(LSTM(64, return_sequences=True, kernel_regularizer=l2(0.001)), name="bidirectional_lstm1")(x)
    x = BatchNormalization(name="batch_norm_lstm1")(x)
    x = Dropout(0.4, name="dropout_lstm1")(x)  

    x = Bidirectional(LSTM(64, return_sequences=True, kernel_regularizer=l2(0.001)), name="bidirectional_lstm2")(x)
    x = BatchNormalization(name="batch_norm_lstm2")(x)
    x = Dropout(0.4, name="dropout_lstm2")(x)  

    attention_vector = Dense(128, activation='tanh', name="attention_dense1")(x)
    attention_weights = Dense(1, activation='sigmoid', name="attention_weights")(attention_vector)
    attention_weights = Lambda(lambda x: x + 1e-7, name="epsilon")(attention_weights) 
    x = Multiply(name="multiply")([x, attention_weights])

    x = GlobalAveragePooling1D(name="global_avg_pooling")(x)

    x = Dense(128, activation='relu', kernel_regularizer=l2(0.001), name="dense1")(x)
    x = BatchNormalization(name="batch_norm_dense1")(x)
    x = Dropout(0.5, name="dropout_dense1")(x) 

    x = Dense(64, activation='relu', kernel_regularizer=l2(0.001), name="dense2")(x)
    x = BatchNormalization(name="batch_norm_dense2")(x)
...
class SignLanguageDataGenerator(tf.keras.utils.Sequence):
    ...
    """
    Data generator for variable-length input sequences.

    Args:
        X (list): List of input feature sequences.
...
def main2():
    ...
    X_data, y_labels = extract_features_threaded(base_dir, feature_vectors_dir, max_workers=3)
